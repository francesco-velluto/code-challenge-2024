...
def parse_input(file_name):
    with open(file_name, 'r') as f:
        ...
        silver_points = {tuple(map(int, f.readline().split()[:2])): int(f.readline().split()[2]) for _ in range(SM)}
...
def main():
    ...
    optimal_path = find_optimal_path(W, H, GN, SM, TL, golden_points, silver_points,tiles)
